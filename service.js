...
const parseSams = async (sku) => {
    let res = samsClub(sku)
    res = await promiseResolver(res);

    try {
        res = await res.json();
        let aval = res.payload.products[0].skus[0].onlineOffer.inventory.status;
        if (aval !== "NOT_AVAILABLE"){
            open("https://www.samsclub.com/p/asdf/" + sku);
            player.play('alarm.mp3', { timeout: 300 }, function(err){
                if (err) throw err
            })
            return 69;
...
const bestbuy = (sku) => {
    ...
    return fetch(url + sku, {
        headers: {...headers}
    });
}

const samsClub = (productId) => {
    return fetch("https://www.samsclub.com/api/node/vivaldi/v2/az/products", {
        "headers": {
        ...
        "body": "{\"productIds\":[\""+ productId +"\"],\"type\":\"LARGE\",\"clubId\":4936}",
        "method": "POST",
        "mode": "cors"
    });
}
